...
public interface ExclusionWithMetadata extends ExclusionWithoutMetadata {
	 ...
	 *
	 * <br>All metadata records associated with the destination entity will also be removed.
	 *
	...
	/**
	 * Configures the {@link EntityMapping} to remove records from the destination entity when the source record no longer exists.
	 *
	 * <p>This depends on metadata:
	 * <ul>
	 * 	<li>A data mapping cycle will be executed by the {@link DataIntegrationEngine} and in the first execution,
	 * 	the identifiers of the source entity will be stored in the metadata, associated with the identifiers of each row mapped in the destination.</li>
