 ...
 *
 * <p>Use {@link DataIntegrationEngine#addFunction(EngineScope, String, FunctionCall)} to associate it with a name and a scope.
 ...
 *    concurrent function invocations might happen. In this situation it is important to avoid sharing an object with multiple functions; or synchronize any code that alters its state.
 *
 * <p>Calls executed within uniVocity will cache the function result in the given scope and will only be executed again once data stored in that scope is lost.
 *
 * <p>Functions can be used to transform mapped values. They are expressed in mappings using special expressions. For example, consider the function <i>getLocaleId</i>:
 *
 * <p><hr><blockquote><pre>
 ...
 * enclosed within curly brackets. The following example uses it to obtain the <i>en_AU</i> locale ID in a mapping using different constructs:
 * <p><hr><blockquote><pre>
 ...
 * </pre></blockquote><hr>
 *
 * <p> There is no limitation on the number of arguments used to invoke a function. If a function takes multiple arguments, use an array of Objects.
 *
 * @see FunctionWrapper
 * @see EngineScope
